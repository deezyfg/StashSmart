        ...
        container.innerHTML = `
            <div class="health-overview">
                <div class="health-status ${this.healthData.status}">
                    <h2>System Status: <span class="status-badge ${this.healthData.status}">${this.healthData.status.toUpperCase()}</span></h2>
            ...
            <div class="health-actions">
                ...
                <button onclick="healthMonitor.toggleAutoRefresh()" class="btn btn-info">
                    <i class="fas fa-clock"></i> ${this.updateInterval ? 'Stop' : 'Start'} Auto-Refresh
                </button>
            </div>
        `;
    }
            ...
            <div class="health-card ${db.status}">
                ...
                <div class="card-body">
                    ...
                    ${db.missing_tables && db.missing_tables.length > 0 ? 
                        `<p class="warning"><strong>Missing Tables:</strong> ${db.missing_tables.join(', ')}</p>` : ''
                    }
                </div>
            </div>
        `;
    ...
    renderApiHealth() {
        ...
        return `
            <div class="health-card ${api.status}">
                ...
                <div class="card-body">
                    <p><strong>Healthy:</strong> ${api.healthy_endpoints}/${api.total_endpoints}</p>
                    <div class="endpoint-list">
                        ${Object.entries(api.details).map(([endpoint, data]) => `
    ...
    renderUserActivity() {
        ...
        return `
            <div class="health-card ${activity.status}">
                ...
                <div class="card-body">
                    <p><strong>Active (24h):</strong> ${activity.active_users_24h || 0}</p>
                    <p><strong>Active (7d):</strong> ${activity.active_users_7d || 0}</p>
                    <p><strong>Total Users:</strong> ${activity.total_users || 0}</p>
                    <p><strong>Transactions (24h):</strong> ${activity.transactions_24h || 0}</p>
                    <p><strong>Engagement Rate:</strong> ${activity.user_engagement_rate || 0}%</p>
                </div>
            </div>
        `;
    }
    ...
    renderFileSystemHealth() {
        ...
        return `
            <div class="health-card ${fs.status}">
                ...
                <div class="card-body">
                    <p><strong>Disk Usage:</strong> ${fs.disk_usage_percent}%</p>
                    <p><strong>Free Space:</strong> ${fs.free_space_mb} MB</p>
                    <div class="directory-list">
                        ${Object.entries(fs.directories).map(([dir, data]) => `
                            <div class="directory-item ${data.exists && data.readable ? 'ok' : 'error'}">
                                <span class="dir-name">${dir}</span>
                                <span class="permissions">
                                    ${data.readable ? 'R' : ''}${data.writable ? 'W' : ''}
                                </span>
                            </div>
                        `).join('')}
                    </div>
            ...
            <div class="stats-grid">
                <div class="stat-card">
                    ...
                    <div class="stat-info">
                        ...
                        <p>Total Users</p>
                        <small>${stats.users.new_users_30d} new this month</small>
                    </div>
                </div>
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-info">
                        ...
                        <small>${stats.transactions.transactions_30d} this month</small>
                    </div>
                </div>
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-info">
                        <h3>$${parseFloat(stats.accounts.total_balance || 0).toLocaleString()}</h3>
                        <p>Total Balance</p>
